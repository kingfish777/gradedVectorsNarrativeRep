...
for (c in cases[1]) {
  ...
  n <- as.integer(.7*length(as.vector(unlist(c, recursive=FALSE)[4])[[1]])) #^2
  ...
  for (m in 1:n) {
    ...
    for (g in gv) {
      ...
      for (tr in training) {
        ...
        tale.idx <- tale.idx[1:as.integer(length(tale.idx)*.75)]
        ...
        taleNames <- getIndex(unlist(c[3]), tale.idx)
        
        taleNames <- substr(taleNames, start=0, stop=(nchar(taleNames) -4))
        ...
        if (tr == "BNC") {
          ...
          if (g == "1") {
            ...
            system("java -cp ../semanticvectors/target/semanticvectors-5.9.jar pitt.search.semanticvectors.orthography.SentenceVectors -luceneindexpath tale_index -initialtermvectors ../bnc/termtermvectors.bin -vectortype binary -dimension 32000 -elementalmethod contenthash -termweight idf")
            ...
            system(" java -cp ../semanticvectors-5.9.jar pitt.search.semanticvectors.CompareTermsBatch -queryvectorfile docvectors.bin < afanInput.txt &> output.txt")
          ...
          if (g == "1") {
            ...
            system(" java -Xmx60G -cp ../semanticvectors/target/semanticvectors-5.9.jar pitt.search.semanticvectors.BuildIndex -luceneindexpath tale_index -vectortype binary -minfrequency 1 -filteroutnumbers -maxnonalphabetchars -1 -maxfrequency 100000000 -dimension 32000 -termweight idf -initialtermvectors random -trainingcycles 2 -elementalmethod contenthash -docindexing incremental -stoplistfile smartstop.txt")
            
            #genSentVecLemBNC.sh
            
            print("building graded sentencevectors with probabilistic normalization")
            
            system("java -cp ../semanticvectors/target/semanticvectors-5.9.jar pitt.search.semanticvectors.orthography.SentenceVectors -luceneindexpath tale_index -initialtermvectors random -vectortype binary -dimension 32000 -elementalmethod contenthash -termweight idf")
            ...
            #cor.test(hclust(dist(dm), method = "single"), hclust(dist(res.matSMPlem), method = "single"))
        ...
        # build input file for this case
