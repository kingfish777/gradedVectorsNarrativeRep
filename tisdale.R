...
#genROCs <- function(condition, pathway, em) {
  ...
  for (controlInd in controlInds) {
    ...
    for (dataPointPath in listPaths) {
      ...
      print(paste0(condition, controlInd, path, poison, medCoef, medCoef0, converged, ixn, medCnt, zScore, zScore0, collapse = ": "))
      dataRow <- c(condition, controlInd, path, poison, medCoef, medCoef0, converged, ixn, medCnt, zScore, zScore0)
  ...
  colnames(data.tbl) <- c("adr", "controlInd", "path", "med", "medCoef", "medCoef0", "converged", "ixn", "medCnt", "zScore", "zScore0")
  colnames(data) <- c("adr", "controlInd", "path", "med", "medCoef", "medCoef0", "converged", "ixn", "medCnt", "zScore", "zScore0")
  ...
  cond_data_all <- subset(cond_data_all, medCnt > 1000)
  cond_data_tis <- subset(cond_data_all, subset = med %in% mi_tisdale)
  print("cond_data_tis")
  print(cond_data_tis)
  ...
  dm.tbl$medCoefDiff <- as.numeric(dm.tbl$medCoef0) - as.numeric(dm.tbl$medCoef)
  dm.tbl$medZvalDiff <- as.numeric(dm.tbl$zScore0) - as.numeric(dm.tbl$zScore)
  dm.tbl
  fp.subset <- subset(dm.tbl, subset = controlInd == 0)
  #fp.subset$rankOrderDiff <- rank(fp.subset$medCoefDiff, na.last=TRUE, ties.method="max")
  fp.subset$rankzDiff <- rank(fp.subset$medZvalDiff, ties.method="max")
  max.fp.subset.glmDiff <- max(fp.subset$rankOrderDiff)
  fp.subset.glmDiff.cutoff <- max.fp.subset.glmDiff - round(.2*max.fp.subset.glmDiff,0)
  fp.subset.bestperformers <- subset(fp.subset, subset = rankOrderDiff >= fp.subset.glmDiff.cutoff)
  fp.subset.bestperformers$med
  
  fp.subset$rankzDiff <- rank(fp.subset$medZvalDiff, ties.method="max")
  max.fp.subset.zDiff <- max(fp.subset$rankzDiff)
  fp.subset.zDiff.cutoff <- max.fp.subset.zDiff - round(.2*max.fp.subset.zDiff,0)
  fp.subset.bestperformers <- subset(fp.subset, subset = rankzDiff >= fp.subset.zDiff.cutoff)
  fp.subset.bestperformers$med
